...
# Open CSV file and define headers
with open(csv_filename, mode="w", newline="") as file:
    ...
    for packet in capture.sniff_continuously(packet_count=200):  # Capture 20 packets
        try:
            ...
            src_ip = packet.ip.src if hasattr(packet, 'ip') else (packet.ipv6.src if hasattr(packet, 'ipv6') else "N/A")
            dst_ip = packet.ip.dst if hasattr(packet, 'ip') else (packet.ipv6.dst if hasattr(packet, 'ipv6') else "N/A")

            # Get Packet Length
            packet_length = packet.length if hasattr(packet, "length") else "Unknown"

            # Initialize variables
            service, src_port, dst_port, tcp_flags, checksum, tcp_window_size , icmp_type = "Unknown", "N/A", "N/A", "0", "N/A", "N/A" , "0"

            # Check if it's a TCP or UDP or ICMP packet
            if hasattr(packet, 'tcp'):
                ...
                checksum = f"0x{packet.tcp.checksum}" if hasattr(packet.tcp, "checksum") else "N/A"

                # Extract TCP Window Size
                tcp_window_size = packet.tcp.window_size if hasattr(packet.tcp, "window_size") else "N/A"
                ...
                checksum = f"0x{packet.udp.checksum}" if hasattr(packet.udp, "checksum") else "N/A"
